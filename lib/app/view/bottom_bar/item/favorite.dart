 ...
 Dio? dioDownload;
  ...
  getWaFavorite(){
    SchedulerBinding.instance?.addPostFrameCallback((_) {
      if(mounted){
        Provider.of<WaFavoriteNotif>(context, listen: false).getEbooksFavorites();
        Provider.of<WaDetail>(context, listen: false).checkFav(notif.list.length);
      }
    });
  }
  ...
  Widget build(BuildContext context) {
    return Consumer<WaFavoriteNotif>(
      builder: (context, waFavoriteNotif, _){
                      ...
                      children: [
                          ...
                          children: [
                            GestureDetector(
                              child: Container(
                                ...
                                decoration: BoxDecoration(
                                    borderRadius: BorderRadius.all(Radius.circular(10)),
                                    color: waFavorite.color == "" ? getColorFromHex(GlobalColors().colorWhite) :
                              ...
                              onTap: (){
                                pushPage(context, WaStickerDetail(pack: waFavorite,));
                              },
                            ),
                            SizedBox(width: 8,),
                            GestureDetector(
                              child: Column(
                                ...
                                children: [
                                  Text(waFavorite.publisher, style: TextStyle(
                                      color: Colors.black, fontSize: 18
                                  ),),
                                  Text(waFavorite.name, style: TextStyle(
                            ...
                            Stack(
                              children: [
                                if(!downloadList.contains(waFavorite.identiFier))...[
                        ...
                        Hero(
                          ...
                          child: GestureDetector(
                            child: Row(
                              children: [
                                for(int i = 0; i < waFavorite.sticker.length; i++) if(i < 6) Flexible(
                                  child: Padding(
  ...
  Future<void> downloadStickers(StickerPack pack) async{
    ...
    if(!downloadList.contains(pack.identiFier)){
      ...
      String tray = trayImgPath.path+basenamepath.basename(pack.trayimageFile);
      ...
      for(int i = 0; i<pack.sticker.length; i++){
        String pathFile = paths.path + basenamepath.basename(pack.sticker[i].imageFile);
